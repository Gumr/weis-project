...
VantComponent({
    ...
    methods: {
        ...
        setColumnValues(index, options, needReset = true, value) {
            ...
            if (needReset == 'date') {
                if (isSame) {
                    return Promise.resolve(options.indexOf(value));
                } else {
                    return column.set({ options }).then(() => {
                        if (options.indexOf(value) >= 0) {
                            column.setIndex(options.indexOf(value));
                            return Promise.resolve(options.indexOf(value));
                        } else {
                            // if (Number(value.replace('日', '')) < Number(options[0].replace('日', ''))) {
                            //     column.setIndex(0);
                            //     return Promise.resolve(0);
                            // } else {
                            //     column.setIndex(options.length - 1);
                            //     return Promise.resolve(options.length - 1);
                            // }
                            column.setIndex(0);
                            return Promise.resolve(0);
                        }
