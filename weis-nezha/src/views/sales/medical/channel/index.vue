      ...
      <div>
        ...
        <div class="section-item">
          <span class="section-label label-1">
            <span style="color:red">*</span>渠道名称：
          </span>
        ...
        <div class="section-item">
          ...
          <BaseSelect class="medium-input" :options="options" v-model="currentTag.tpiChannelType"></BaseSelect>
        </div>
        <div class="section-item">
          <span class="section-label label-1">
...
export default {
  ...
  methods: {
    ...
    async getManager() {
      ...
      if (!res.errMsg) {
        this.optionsB = res.obj.map(val => ({label: val.tpiName + ' ' + val.tpiPhone, value: val.tpiId}));
      }
    },
    async getList() {
      const params = {
    ...
    async tagConfirm(done) {
      if (!this.currentTag.tpiName) {
        this.$msg('请输入医疗渠道经理名称', 'error');
        done();
        return;
      }
      if (!this.currentTag.tpiPhone) {
        this.$msg('请输入医疗渠道经理手机', 'error');
        done();
        return;
      }
