  ...
  >
    ...
    >
        ...
        <div>
          ...
          >
            ...
            <el-form-item
              ...
              prop="wxNumber"
            >
              <el-input v-model="coach.wxNumber" />
            </el-form-item>
            <el-form-item
              label="手机号"
              prop="coachPhone"
      ...
      <template #example>
        ...
        >
          ...
          <el-table-column
            ...
            label="效果"
          >
            <template #default="{ row }">{{ row.drillBefore }}斤-{{ row.drillAfter }}斤</template>
          </el-table-column>
          <el-table-column
            key="example"
            label="操作"
      ...
      <template #comment>
        ...
        >
          <el-table-column
            v-for="(col, index) in comment.cols"
            :key="col.prop"
            v-bind="col"
          >
            <template
...
export default defineComponent({
  ...
  methods: {
    ...
    handleInfoConfirm(loading) {
      this.$refs.form.validate((valid) => {
        if (valid) {
          const done = loading();
          this.$request("coach.BasicInfo/addBasiclnfo", this.coach)
            .thenwrap((err) => {
              if (!err) {
    ...
    async editClick(row) {
      const res = (await this.getGetCoachInfo(row.recordId)).data.obj;
      Object.assign(this.coach, res);
      this.tab = "info";
    ...
    headPortraitChange({ type, index, value }) {
    ...
    getGetCoachInfo(recordId) {
      return this.$request("coach.BasicInfo/getGetCoachInfo", { recordId });
    },
    changeStatusClick(row, status) {
      ...
      this.$request(url, { recordId: row.recordId }).thenwrap((err) => {
        if (!err) {
          this.$message.success("操作成功");
          this.getList();
        } else {
    ...
    async getList() {
      ...
      const res = await this.$http("coach.BasicInfo/queryGetCoach", {
        ...this.page,
        ...this.queryParams,
      });
      if (!res.errMsg) {
