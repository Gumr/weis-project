    ...
    >
      <div>
        <div class="section-item">
          <span class="section-label label-1">渠道主管手机：</span>
          <el-input
            clearable
            class="medium-input"
            maxlength="11"
        ...
        <div class="section-item">
          <span class="section-label label-1">渠道主管昵称：</span>
          <span>{{currentTag.uname}}</span>
        </div>
        <div class="section-item">
          <span class="section-label label-1">渠道主管名称：</span>
          <el-input clearable class="medium-input" v-model="currentTag.tpiName"></el-input>
        </div>
        <div class="section-item">
          <span class="section-label label-1">渠道总监：</span>
...
export default {
  ...
  methods: {
    async getManager() {
      ...
      if (!res.errMsg) {
        this.optionsB = res.obj.map(val => ({label: val.tpiName, value: val.tpiId}));
      }
    },
    async getList() {
      const params = {
