  ...
  <div class="page-container">
    <section>
      ...
      <div v-if="tableData.length" class="section-item">
        ...
        <span style="white-space: pre-wrap;">{{nutrition}}</span>
      
      </div>
        <span>菜品数量 * {{ dishTotal }} ，菜品总价：{{ priceTotal }} 元</span>
      <div class="section-item">
      ...
      <div v-if="tgcImgUrl" class="section-item">
        ...
        <el-image :src="tgcImgUrl" style="width: 200px; height: 200px;" fit="contain" />
      </div>
      <div class="section-item">
...
export default {
  ...
  data() {
    return {
      ...
      tableCol: [
        ...
        { label: '营养元素', prop: 'nutrient', width: '120' },
        { label: '销售状态', prop: 'tfsSttDesc', width: '120' },
        { label: '供应时间', prop: 'sellDate', width: '120' },
  ...
  methods: {
    async getInfo() {
      ...
      this.nutrition = res.obj.nutrition
      this.tableData = res.obj.skuInfo
      this.tgcClassifyDesc =res.obj.tgcClassifyDesc
      this.tgcImgUrl = Array.isArray(res.obj.tgcImgUrl)
        ? (res.obj.tgcImgUrl[0] && res.obj.tgcImgUrl[0].imgUrl)
        : res.obj.tgcImgUrl
      this.tgcCrowd = res.obj.tgcCrowd.length>0?res.obj.tgcCrowd.map((item) => {
       return item+''
      }):[],
