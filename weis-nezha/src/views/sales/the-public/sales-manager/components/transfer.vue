<template>
  <div class>
    <div style="margin-top: 20px;" v-if="tab == 0">
    ...
    <div class="tree-select-wrap display-flex">
      <div class="tree-select-section">
        <h3 v-if="tab == 0">{{counselor}}的客户名单</h3>
        <h3 v-else>{{counselor}}的销兼职客户经理名单</h3>
        <div class="tree-box">
          <BasePageTable
      ...
      <div class="tree-select-control" v-if="activeRadio == '0101'">
      ...
      <div class="tree-select-section" v-if="activeRadio == '0101'">
        <h3>{{counselorType == '01' ? '目标全职客户经理/兼职客户经理名单' : (tab == '0' ? '目标全职客户经理/兼职客户经理名单' : '目标全职客户经理名单') }}</h3>
        <div class="tree-box">
          <el-select
...
export default {
  ...
  created() {
    ...
    this.tableHeight = `${height - 540}px`;
    this.tableHeight2 = `${height - 640}px`;
    this.counselorId = this.$route.query.counselorId;
    this.id = this.$route.query.id;
    this.counselor = this.$route.query.name;
    this.counselorType = this.$route.query && this.$route.query.counselorType;
    this.getList();
    this.getCounselor();
  },
  methods: {
    counselorChange(e) {
      ...
      this.counselorTypeDesc = item.counselorType === '02' ? '自有' : '外部';
    },
    radioChange() {
      this.tableCounselor = [];
      this.middelOptions = [];
    ...
    getCounselor() {
      ...
      if (this.tab == '0') {
        delete params.counselorType;
      }
      this.$request('Channel/queryCounselorAll', params).then(
        this.$rw((err, dataPage) => {
          if (!err) {
    ...
    confirm() {
      this.$confirm('是否提交？', '提示', {
        ...
        const type = this.type === '01' ? this.activeRadio : '0201';
        // if (this.tab == '1') {
        //   type = '0201';
        // }
        const params = {
        ...
        if (type === '0101' || type === '0201') {
          ...
          params.transferInfo[''] = [];
          this.tableSelection.forEach((item) => {
            params.transferInfo[''].push(item.counselorId);
          });
