...
export default {
  ...
  methods: {
    ...
    async onconfirm(done) {
      ...
      const res = await this.$http('Channel/bandingHelper', { id: this.id, marketType: '01', uid: this.current.uid })
      if (!res.errMsg) {
    ...
    async transfer() {
      ...
      this.targetId = ''
      const res = await this.$http('Channel/queryOwnCounselor', { marketType: '01', status: '03', channel: '01,02' })
    ...
    async transferAllHeplper() {
      ...
      const res = await this.$http('Channel/queryHelperList', { counselorId: this.$route.query.counselorId, marketType: '01' })
