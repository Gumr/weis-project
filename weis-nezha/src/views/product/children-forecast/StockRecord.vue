  ...
  <div class="page-container">
    <div class="query-bar">
      ...
      >
        ...
        <template #action>
          <el-button type="primary" @click="batchSync">批量同步</el-button>
          <!-- <el-button type="text" @click="downLoad(1)">下载端外备货模板</el-button> -->
        </template>
      </QueryComponents>
      <input
        ref="upload"
        ...
        style="opacity: 0;"
      />
      <el-button type="primary" @click="toEnter('add')">备货录入</el-button>
    </div>
    <div>
      <BasePageTable
        ref="table"
  ...
  methods: {
    ...
    toEnter(type, row) {
      if ((row && row.tkdhType) === '01') {
        ...
        this.$router.push({
          path: `${this.$route.path}/foodEnter`,
          query: {
            tkdhId: type == 'add' ? null : row.tkdhId,
            date: type == 'add' ? null : row.tkdhStockDate,
            heatPointId: type == 'add' ? null : row.tkdhHpid
    ...
    deleteClick(row) {
      this.$request('OrderStockHist/deleteStockHist', { tkdhId: row.tkdhId, type: '01' }).then(
        this.$rw((err) => {
          if (!err) {
      ...
      fileReader.onload = async (ev) => {
        try {
          ...
          for (const wsname of workbook.SheetNames) {
            ...
            for (const w of ws) {
              if (w['备货量'] && w['用途'] != '小程序' && w['用途'] != '美团' && w['用途'] != '市场活动' && w['用途'] != '质检留样' && w['用途'] != '其他') {
                this.$msg('请输入正确的用途', 'error')
                this.reload()
                return
              }
            }
            const arr1 = ws.filter((val) => val['用途'] == '小程序')
            ...
            const stockArr = [arr1, arr2, arr3, arr4, arr5]
            const arrtype = ['01', '02', '03', '04', '05']
            const tkdhHpid = this.queryComps[1].props.options.find((val) => val.label == wsname).value
            for (const i in stockArr) {
              if (stockArr[i].length) {
                ...
                let stockList = {}
                if (arrtype[i] == '01') {
                  stockList = stockArr[i].map((val) => ({ tfsCid: val['编码'], tkdhInsideReserveTotal: val['预定单备货量'], tkdhInnerTotal: val['现点备货量'] }))
                } else {
                  stockList = stockArr[i].map((val) => ({ tfsCid: val['编码'], outsideStock: val['备货量'] }))
                }
                const remark = stockArr[i].find((val) => val['备注'])
                const params = {
                  tkdhHpid,
                  date: this.$day(`${stockArr[i][0]['备货使用日期']}`).format('YYYY-MM-DD'),
          ...
          const res = await this.$http('OrderStockHist/updateStockHistImport', { pointStockList })
          if (!res.errMsg) {
    ...
    async downLoad(type) {
      const res = await this.$http('Sku/querySkuList', { pageNo: 1, pageSize: 9999, tfsStt: 60 })
      const date = this.$day().hour() >= 18 ? new Date(Date.now() + 86400000 * 2) : new Date(Date.now() + 86400000)
      const col = type === 0
        ? [
          { label: '序号', type: 'index' },
          ...
          { label: '备货使用日期', formatter: () => this.$day(date).format('YYYYMMDD') }
        ]
        : [
          { label: '序号', type: 'index' },
          { label: '编码', prop: 'tfsCid' },
