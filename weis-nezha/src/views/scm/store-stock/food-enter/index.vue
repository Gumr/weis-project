...
import ATMFood from "./ATM/ATMFood.vue";
      ...
      fileReader.onload = async (ev) => {
        try {
          ...
          for (const wsname of workbook.SheetNames) {
            ...
            const arr1 = ws.filter(
              (val) => val["用途"] == "小程序+美团+饿了么"
            );
              ...
              if (stockArr[i].length) {
                ...
                if (arrtype[i] == "10") {
                  stockList = stockArr[i].map((val) => ({
                    tfsCid: val["编码"],
                    miniStock: val["小程序现点备货量"], //小程序备货量
                    insideReserveStock: val["小程序预订量"], //小程序预定备货量
                    meituanStock: val["美团备货量"],
                    elemeStock: val["饿了么备货量"],
                  }));
                } else {
                  stockList = stockArr[i].map((val) => ({
                    tfsCid: val["编码"],
                    outsideStock: val["端外备货量"],
                  }));
                }
                const remark = stockArr[i].find((val) => val["备注"]);
                const params = {
    ...
    async downLoad(type) {
      ...
      const col =
        ...
        [
          ...
          { label: "菜品名称", prop: "tfsSuggestedSkuname" },
          { label: "用途", prop: "tkdhChannel" },
          { label: "小程序现点备货量", prop: "miniStock" },
          { label: "小程序预订量", prop: "insideReserveStock" },
          { label: "美团备货量", prop: "meituanStock" },
          { label: "饿了么备货量", prop: "elemeStock" },
          { label: "端外备货量", prop: "outsideStock" },
          { label: "备注", prop: "remark" },
          {
            label: "备货使用日期",
            formatter: () => this.$day(date).format("YYYYMMDD"),
