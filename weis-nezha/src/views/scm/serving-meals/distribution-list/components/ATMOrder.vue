    ...
    <div class="query-bar">
      ...
      >
        <template #action>
          ...
          <el-button type="warning" @click="batchPrintClick">批量打印</el-button>
        </template>
      </QueryComponents>
    </div>
    <div>
...
export default defineComponent({
    ...
    return {
      ...
      tableCol: [
        ...
        },
        {
          label: '菜品',
          prop: 'skuname',
          formatter: (row) => (row.skuname.length > 20 ? `${row.skuname.substring(0, 20)}...` : row.skuname)
        },
        {
          label: '菜品总数',
          prop: 'tdodnum'
  ...
  methods: {
    ...
    async distriStatus(row) {
      const res = await this.$http('ServeMealsOperation/shipRecord', { tsoId: row.tdotTradeNo })
    ...
    async getList() {
      ...
      const res = await this.$http('ServeMealsOperation/getDispatchingList', { ...this.page, ...this.genQueryParams() })
    ...
    async handleExport() {
      ...
      const res = await this.$http('ServeMealsOperation/getDispatchingList', { pageNo: 1, pageSize: this.tableDataTotal, ...this.genQueryParams() })
      exportExcel({
    ...
    async goPrint(row) {
      const res = await this.$http('ServeMealsOperation/orderStt', { orderId: row.tdotOrderId })
      if (!res.errMsg) {
        const printData = await this.$http('ServeMealsOperation/orderData', { orderid: [row.tdotOrderId] })
    ...
    async batchPrintClick() {
      ...
      const printData = await this.$http('ServeMealsOperation/orderData', { orderid: sortList })
