...
export default {
  ...
  data() {
    return {
      ...
      table: {
        ...
        col: [
          { type: 'index', label: '序号'},
          { label: '渠道经理ID', prop: 'parentId' },
          { label: '渠道经理名称', prop: 'parentName' },
          { label: '渠道经理手机', prop: 'parentPhone' },
          { label: '渠道兼职客户经理ID', prop: 'counselorId' },
          { label: '渠道兼职客户经理', prop: 'counselorName' },
          { label: '渠道兼职客户经理手机', prop: 'counselorPhone' },
          { label: '渠道兼职客户经理营业额', prop: 'turnoverTotal', formatter: row => `￥${row.turnoverTotal || 0}` },
          { label: '渠道兼职客户经理充值额', prop: 'rechargeTotal', formatter: row => `￥${row.rechargeTotal || 0}` },
          { label: '渠道兼职客户经理微信额', prop: 'wechatTotal', formatter: row => `￥${row.wechatTotal || 0}` },
        ]
      },
      queryParams: {
        marketType: '02',
  ...
  methods: {
    ...
    goDetail(row) {
      ...
      this.$router.push({
        path: '/data/data/commission-statistics/sales-statistics/index/channelSalesMemberDetail',
        query: {
