  ...
  <div class="page-container">
    <section>
      ...
      <div class="section-item">
        ...
        <el-input v-model="formData.name" class="medium-input" placeholder="请输入活动名称"></el-input>
        <!-- <span>{{ formData.name }}</span> -->
      </div>
      <div class="section-item">
        <span class="section-label label-1">活动类型：</span>
        <span>链接/二维码发券</span>
      </div>
      <div class="section-item">
    ...
    <section>
      ...
      <div>
        <div v-for="(item, index) in formData.acquireCondition" :key="index" class="section-item">
          <span class="section-label label-1">发奖券：</span>
          ...
          ></el-input>
          <span style="margin-right: 15px;">张</span>
          <span>人群为：</span>
    ...
    <section>
      ...
      <div v-for="(item ,index) in checkData" :key="index" class="section-item">
        <span class="section-label label-1" style="width: 80px;"></span>
    ...
    >
      ...
      <div v-if="dateRadio == 2" class="section-item">
        ...
        <date-picker v-model="current.takeEffect" class="medium-input" value-format="YYYY-MM-DD"></date-picker>
      </div>
      <div class="section-item">
...
export default {
  ...
  methods: {
    ...
    async getMarket() {
      ...
      this.checkData = res.obj.market.map(val => ({
        label: val.label,
        value: val.value,
        check: false
      })).filter(portFilter)
      ...
      this.$request('coupon.Hotlist/getActivityInfo', { activityId: this.activityId }).then(
        this.$rw((err, res) => {
          if (!err) {
            ...
            this.formData.acquireCondition.forEach((item) => {
             if (item.userPhone) {
                item.userPhone = Array.isArray(item.userPhone) && item.userPhone.join('，')
             }
    ...
    submit() {
      ...
      params.acquireCondition.forEach((item) => {
        ...
        item.userPhone = item.userPhone.toString()
        const lastChart = item.userPhone.charAt(item.userPhone.length - 1)
        if (lastChart == '，' || lastChart == ',') {
          item.userPhone = item.userPhone.substring(0, item.userPhone.length - 1)
        }
        item.userPhone = item.userPhone.replace(/\，/g, ',')
        item.userPhone = item.userPhone.split(',').map(String)
      })
    ...
    handleSubmit(params) {
      let url = 'coupon.Hotlist/addlinkorqrcodeSendCoupon'
      if (this.activityId) {
    ...
    readExcel(e) {
      ...
      fileReader.onload = (ev) => {
        try {
          ...
          const lastChart =userPhone.length>0? userPhone.charAt(userPhone.length - 1):''
