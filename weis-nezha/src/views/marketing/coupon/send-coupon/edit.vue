    ...
    <section>
      ...
      <div>
        ...
        >
          ...
          <span>已导入 {{item.userPhone.length}} 人</span>
          <el-button
            type="success"
            style="margin-left:20px ;margin-right:20px;"
            @click="showUser(item.userPhone,index)"
          >查看人群</el-button>
          <!-- <span>人群为：</span>
          <el-input
    ...
    >
      <el-table :data="userList" style="width: 100%">
        ...
        <el-table-column label="手机号" align="center">
          <template v-slot="{ row }"><span>{{row}}</span></template>
        </el-table-column>
        <el-table-column
          label="操作"
          align="center"
...
export default {
  ...
  methods: {
    ...
    showUser(puhone,index) {
   
      this.userList = puhone;
      this.userIndex = index
      this.showUserDil = true;
    },
    checkText() {
    ...
    // 获取优惠券
    async getCoupon() {
      const res = await this.$http("coupon.Coupon/getCouponAll", {
        pageNo: 1,
        pageSize: 999,
      });
    ...
    toDel(index)
    {
         this.formData.acquireCondition[this.userIndex].userPhone.splice(index, 1);
    },
    ...
    submit() {
      ...
      params.acquireCondition.forEach((item) => {
        ...
        //   item.userPhone = item.userPhone.substring(
        //     0,
        //     item.userPhone.length - 1
        //   );
        // }
        //   debugger
    ...
    readExcel(e) {
      ...
      fileReader.onload = (ev) => {
        try {
          ...
          this.formData.acquireCondition[this.uploadIndex].userPhone =
            this.formData.acquireCondition[this.uploadIndex].userPhone.concat(outputs);
          this.formData.acquireCondition.push({});
