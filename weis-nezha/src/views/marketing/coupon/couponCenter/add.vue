    ...
    <section>
      ...
      >
        ...
        <div class="section-item">
          <div class="coupon">
            <div
              v-for="(item2, index) in item.coupon"
              :key="index"
              class="coupon-item"
            >
              <div class="c-item">
...
export default {
  ...
  methods: {
    couponDel(index) {
      ...
      const guid = this.formData.acquireCondition[isindex].coupon[index].guid;
      ...
      this.formData.acquireCondition[isindex].coupon.splice(index, 1);
      this.couponOptions = this.formData.acquireCondition[isindex].coupon.map(
        (item) => ({
          value: item.guid,
    ...
    couponUpd(index) {
      const isindex = this.isindex;
      const item = this.formData.acquireCondition[isindex].coupon[index];
    ...
    tagConfirm(done) {
      ...
      const index = this.formData.acquireCondition[isindex].coupon.findIndex(
        (item) => item.guid == params.guid
      );

      if (index > -1) {
        this.formData.acquireCondition[isindex].coupon[index] = params;
        this.formData.acquireCondition[isindex].coupon.push({});
        this.formData.acquireCondition[isindex].coupon.pop();
      } else {
        this.formData.acquireCondition[isindex].coupon.push(params);
      }
    ...
    submit() {
      ...
      if (!params.acquireCondition.length) {
        this.$message({ type: "error", message: "请添加活动内容！" });
        return;
      }
      if (!params.acquireCondition[0].coupon.length) {
        this.$message({ type: "error", message: "请添加领券内容！" });
        return;
      }
      ...
      console.log(idsSet);
      params.coupon = params.acquireCondition;
      params.coupon.forEach((item, index) => {
        item.activityCoupon = item.coupon;
      ...
      delete params.acquireCondition;
    ...
    handleSubmit(params) {
      let url = "coupon.Hotlist/getCouponCentre";
      this.loading = true;
