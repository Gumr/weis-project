<template>
  <div class="page-container">
    ...
    <div class="query-bar">
      ...
      <el-button @click="addMemberClick" type="primary">添加内部会员</el-button>
      <el-button @click="handleRemove" type="danger">批量移除</el-button>
    </div>
    <div>
      <BasePageTable
        ref="table"
      ...
      >
        ...
        <el-table-column label="注册时间" width="160" prop="ctime">
          <template v-slot="{ row }">{{ $day(+row.ctime).format("YYYY-MM-DD HH:mm:ss") }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template v-slot="{ row }">
            ...
            <div class="op-action brand-color cursor-pointer" @click="remove(row)">移除</div>
          </template>
        </el-table-column>
      </BasePageTable>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      // tableFormatter: {
      //   auth: this.memberClassLabel,
      //   userStatus: this.userStatusLabel,
      //   ctime: row => this.$day(+row.ctime).format('YYYY-MM-DD HH:mm:ss'),
      //   isConsumed: row => (row.isConsumed === '1' ? '是' : '否')
      // },
      queryComps: [
        {
  ...
  methods: {
    ...
    remove(row) {
      this.$request('Customer/updateUserToBeta', {
        phone: [row.phone],
        opType: '0',
        auth: '2'
      }).then(
    ...
    handleRemove() {
      ...
      const arr = this.multipleSelection.map(item => item.phone);
      this.$request('Customer/updateUserToBeta', {
        phone: arr,
        opType: '0',
        auth: '2'
